...
struct BuildCommand: CommandProtocol {
    ...
    struct Options: OptionsProtocol {
        ...
        static func create(_ project: String?) -> (String?) -> (String) -> (String) -> (String?) -> (String?) -> (String?) -> (String?) -> (String?) -> (Bool) -> ([String]) -> Options {
            return { name in { outputDirectory in { buildDirectory in { allScheme in { iOSScheme in { watchOSScheme in { tvOSScheme in { macOSScheme in { verbose in { compilerArguments in Options(project: project, name: name, outputDirectory: outputDirectory, buildDirectory: buildDirectory, allScheme: allScheme, iOSScheme: iOSScheme, watchOSScheme: watchOSScheme, tvOSScheme: tvOSScheme, macOSScheme: macOSScheme, verbose: verbose, compilerArguments: compilerArguments) } } } } } } } } } }
        }
        ...
        static func evaluate(_ mode: CommandMode) -> Result<Options, CommandantError<CommandantError<()>>> {
            ...
            return create
                ...
                <*> mode <| Option(key: "all", defaultValue: nil, usage: "the scheme for all targets")
